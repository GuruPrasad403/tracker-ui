        ...
        <>
            <div className="hidden lg:grid grid-cols-1 grid-rows-12 px-2 bg-gray-300 w-full h-screen shadow-xl">
                <div className="row-span-2 flex justify-start gap-10 px-5 items-center h-full">
                    <div>
                        <img src={image} className='lg:w-20 lg:h-20 md:w-10 md:h-10' alt="User Icon" />
                    </div>
                    <div>
                ...
                <div className="row-span-9 flex flex-col flex-start gap-10 items-center">
                    {
                        MenuItemsList.map((ele, i) => <MenuItems key={i} title={ele.title} icon={ele.icon} navigate={navigate} route={ele.route} />)
                    }
                </div>
                <div>
                    <div className="text-black rounded-md py-2 cursor-pointer" onClick={() => {
                        localStorage.clear();
                        navigate('/login');
                    }}>
...
const MenuItems = ({ title, icon, navigate, route }) => {
    return (
        <div className="hover:bg-violet-400 hover:text-white hover:rounded-xl  flex justify-start gap-5 pl-5 py-2  items-center w-full cursor-pointer" onClick={() => {
            navigate(`/${route}`);
        }}>
